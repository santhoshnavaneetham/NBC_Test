...
public class Log extends BaseTest {
	...
	public static String getSecurityScanHTMLHyperLink(String inputFile) {
		String screenShotLink = "";
		screenShotLink = "<a href=\"." + File.separator + "Security Scan Report" + File.separator + inputFile
	 ...
	 */
	public static void testCaseResultExtentReport(ExtentTest extentedReport, WebDriver driver) {
		if (Reporter.getOutput(Reporter.getCurrentTestResult()).toString().contains("FAILSOFT")) {
			Log.failExtentReport("Test Failed. Check the steps above in red color.", extentedReport, driver);
			fail("Test Failed. Check the steps above in red color.");
		} else {
			Log.passExtentReport("Test Passed.", extentedReport, driver);
			pass("Test Passed. ");
		}
	 ...
	 *            test case
	 */
	public static void messageExtentReport(String description, ExtentTest extentedReport) {
	...
	public static void messageWithExtentScreenshot(String description, WebDriver driver, ExtentTest extentedReport,
		...
		extentedReport.log(LogStatus.INFO, description + extentedReport.addScreenCapture("./ScreenShot/" + inputFile));
	 ...
	 *             - java/selenium exception
	 */
	public static void exceptionExtentReport(Exception e, WebDriver driver, ExtentTest extentedReport)
			throws Exception {
		...
		lsLog4j().log(callerClass(), Level.FATAL, eMessage, e);
		if (e instanceof SkipException) {
			extentedReport.log(LogStatus.SKIP, eMessage + extentedReport.addScreenCapture("./ScreenShot/" + inputFile));
			Reporter.log(SKIP_EXCEPTION_HTML_BEGIN + eMessage + SKIP_HTML_END1 + getScreenShotHyperLink(inputFile)
					+ SKIP_HTML_END2);			
		} else {
			extentedReport.log(LogStatus.FAIL, eMessage + extentedReport.addScreenCapture("./ScreenShot/" + inputFile));
			Reporter.log(
					FAIL_HTML_BEGIN + eMessage + FAIL_HTML_END1 + getScreenShotHyperLink(inputFile) + FAIL_HTML_END2);			
		}
	...
	public static void assertThatForEvent(boolean status, String passmsg, String failmsg, long duration, WebDriver driver) {
		if (!status) {
			Log.fail(failmsg, driver);
		} else {
